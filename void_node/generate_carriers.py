...
def generate_ambient_drone(path, duration=60.0):
...
def generate_harmonic_mix(path, duration=45.0):
    n = int(SAMPLE_RATE * duration)
    t = np.linspace(0, duration, n, endpoint=False)
...
def generate_pink_noise(path, duration=30.0):
    n = int(SAMPLE_RATE * duration)
    t = np.linspace(0, duration, n, endpoint=False)
    ...
    fft_result = np.fft.rfft(white)
    freqs = np.fft.rfftfreq(n, d=1.0 / SAMPLE_RATE)
    freqs[0] = 1.0
    fft_result /= np.sqrt(freqs)
    pink = np.fft.irfft(fft_result, n=n)
