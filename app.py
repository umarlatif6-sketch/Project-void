...
@app.route("/api/journalism/encode", methods=["POST"])
def journalism_encode():
    ...
    payload_path = os.path.join(INPUT_DIR, safe_name)
    ...
    try:
        ...
        output_path = os.path.join(SILT_DIR, output_name)
        ...
        info = analyze_carrier(os.path.join(SILT_DIR, output_name)) if os.path.exists(os.path.join(SILT_DIR, output_name)) else {}

        _log_operation("SILT_JOURNALISM", output_name, hash_key, f"style={style}")
...
@app.route("/api/journalism/decode", methods=["POST"])
def journalism_decode():
    ...
    silt_path = os.path.join(SILT_DIR, secure_filename(filename))
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, safe_out)
...
@app.route("/api/journalism/drops")
def journalism_drops():
    drops = []
    if os.path.isdir(SILT_DIR):
        for f in sorted(os.listdir(SILT_DIR)):
            fp = os.path.join(SILT_DIR, f)
            if os.path.isfile(fp):
                stat = os.stat(fp)
                drops.append({
                    "name": f,
...
@app.route("/api/journalism/download/<filename>")
def journalism_download(filename):
    safe = secure_filename(filename)
    path = os.path.join(SILT_DIR, safe)
    if not os.path.exists(path):
        return jsonify({"error": "File not found"}), 404
    return send_file(path, as_attachment=True)


@app.route("/api/journalism/delete/<filename>", methods=["DELETE"])
...
@app.route("/api/journalism/purge", methods=["DELETE"])
def journalism_purge():
    count = 0
    if os.path.isdir(SILT_DIR):
        for f in os.listdir(SILT_DIR):
            fp = os.path.join(SILT_DIR, f)
            if os.path.isfile(fp):
                os.remove(fp)
                count += 1
