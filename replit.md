...
- **VORTEX Currency (VTX):** Users earn VTX via "Proof of Resonance" (data encoding) and "Proof of Bloom" (mesh relay). Features peer-to-peer transfers and a wallet engine/UI. **VTX Monetization:** Users can buy VTX credit packs via Stripe one-time checkout (Starter=50VTX/£5, Builder=250VTX/£20, Sovereign Stack=1000VTX/£65) and spend VTX on 24-hour feature unlocks (extended_capacity=10VTX, mesh_day_pass=25VTX, journalism_day_pass=15VTX). Idempotent minting via payload_hash guard. Atomic spend+unlock in single DB transaction. Backend: `vortex_wallet.py` (mint_purchase, spend_vtx_with_unlock), `routes/payments.py` (VTX buy/spend/unlocks endpoints), `vtx_unlocks` table. UI: VTX section on `/pricing`, unlock panel in Messenger wallet.
...
- **VORTEX Gifting (Acoustic Gift):** Users gift VTX to other users on specific messages within Messenger. Gift button (◆) appears on received messages. Three visual tiers: Small (<10 VTX) = ψ glyph ripple, Medium (10-99 VTX) = golden σ pulse + ResonanceField, Sovereign (≥100 VTX) = full Silt Gold UI flash + Ω seal + ResonanceField. Generates a 5-second 432 Hz biophony chime WAV (3 tier variants) attached as a gift message. Backend: `void_engine/gift_chime.py` (chime generator), `void_engine/vortex_wallet.py` (`gift_transfer`, `gift` tx_type), `routes/messenger.py` (`POST /api/messenger/gift`), `vortex_gifts` table. Wallet ledger shows Gift Sent/Received with ◆ icons.
