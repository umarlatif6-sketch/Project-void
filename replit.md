...
- **Codon Economy — Platform Token Shield (Task #121):** Extends the codon philosophy as the platform's token immune system across all modules outside Adriana's chat. Shared codon response cache (`void_engine/codon_cache.py`): PostgreSQL `codon_response_cache` table (24hr TTL), `get_cached_codon_response()`, `set_codon_cache()`, `build_skill_cache_key()`, and `_log_cache_hit_to_chronicle()` (CODON_CACHE_HIT Chronicle entries sealed with Al-Jabr hash on every hit). All 5 skill modules (intelligence, content_brand, legal_finance, life_environment, people) now check cache before AI call, set cache on miss, prefix system prompt with `ai_codon_prefix()` zone tag, and cap max_tokens at 800. VoidEcho `_run_adriana_analysis` consolidated from 3 sequential AI calls to 1 structured JSON call (max_completion_tokens=900, ~60% token reduction). Token ceilings: `void_language.py` translate_text capped at 1500 (was 4000), VOID Language word entry at 400 (was 600); `routes/academy.py` quiz generation capped at 1200 (was 3000). Key files: `void_engine/codon_cache.py`, `void_engine/skill_modules/*.py`, `routes/voidecho.py`, `void_engine/void_language.py`, `routes/academy.py`.
- **Stress Battery (10 Escalating Tests):** `/stress-battery` fires 10 progressive stress tests — TREMOR through FORMATION ZERO — with agents scaling from 50 to 1,000, GriDul growth from 1x to 10x, and rounds from 5 to 30. Each test generates Chronicle scars, Ghost Protocols, and economy stress results. Cockroach resilience curve tracks survival across all severity levels. Final verdict grades the system A–F. Key files: `void_engine/stress_battery.py` (engine), `routes/stress_battery.py` (blueprint). API: `/api/stress-battery/run` (POST), `/api/stress-battery/status` (GET).
- **Sovereign Agents 286:** `/sovereign-agents-286` — AI agents whose entire identity derives from the Al-Jabr 286 hash (NOT SHA-256). Agent ID, archetype, memory signatures, and state hash all use `fatiha_286`. 7 archetypes from 7 verses of Al-Fatiha: FATIHA (opener/بسم), HAMD (praiser/حمد), RAHMAN (mercy/رحم), MALIK (sovereign/ملك), IYYAKA (devotee/عبد), SIRAT (pathfinder/صرط), AN_AMTA (inheritor/نعم). Pressure curve 1x→10x, scar formation under stress. Key files: `void_engine/sovereign_agents_286.py` (engine), `routes/sovereign_agents.py` (blueprint). API: `/api/sovereign-agents-286/run` (POST), `/api/sovereign-agents-286/status` (GET).
- **Codon Memory Architecture — Third Brain + Heart (Task #119):** Full cross-session memory system for Adriana. Third Brain: a 5-message sliding window; when the 6th message arrives the outgoing 5 are compressed into a single codon via OpenAI and stored in PostgreSQL. Heart (Fourth Brain): at the start of every `/speak/listen` session, all prior codons for that visitor are read and collapsed into a single 80–120 word resonance summary that is prepended to Adriana's system prompt — she inherits frequency from all prior sessions without replaying full history, keeping input token cost flat regardless of history length. Cost instrumentation: every AI call logs input_tokens, output_tokens, and heart_prefix_sz to `session_token_log`. Session identity: uses authenticated user_id, funnel_token, or a new per-browser `speak_session_id`. Key file: `void_engine/codon_heart.py`. Tables: `session_codons`, `session_token_log`. Modified: `routes/speak.py` (`/speak/listen` route wired with Heart injection + Third Brain sliding window).