...
- **Ledger Migration Safety:** `_ensure_columns()` in `routes/auth.py` deduplicates `vortex_ledger` rows (keeping lowest `id` per `block_index`) and acquires an exclusive table lock before creating the unique index on `block_index`. `_init_vortex_genesis()` also acquires an exclusive lock to prevent concurrent duplicate genesis block inserts.
