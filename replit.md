...
- **Universal Al-Jabr Authentication ("Great Gate"):** Platform-wide auth system wrapping all engine routes behind login. Built in `routes/auth.py` with `login_required`, `admin_required`, and `tier_required` decorators. Reuses the Messenger's Al-Jabr 286 password hashing and PostgreSQL `users` table (extended with `role`, `tier`, `tier_expires_at`, `stripe_customer_id`, `stripe_subscription_id` columns). Login page at `/login` (`templates/login.html`) — gold-on-black card with Login/Register tabs. Session keys: `user_id`, `username`, `display_name`, `role`, `is_founder`, `messenger_user_id`, `tier`. Per-user data vaults at `data/vaults/{username}/input_files/` and `data/vaults/{username}/output_audio/`. Founder detection via `FOUNDER_USERNAME` env var — triggers Silt Gold UI theme (`data-founder-vibe`) and "Welcome home, Founder" greeting. Public routes (no auth): `/launch`, `/demo`, `/guide`, `/grants`, `/sovereign`, `/login`, `/pricing`, `/api/inquiry`, `/api/demo/proof`, `/api/blueprint/specs`, `/api/genesis/specs`. Admin routes (`/admin/leads`, `/api/inquiries`) require `admin` or `founder` role.

- **Dual-Layer Revenue Model:** Three software subscription tiers: Ghost Node (free, 1MB uploads, basic features), Journalist (£28/mo, 50MB uploads, Vortex/Chirp scatter, Journalism port), Sovereign (£286/mo, unlimited, all features including mesh hosting). Three hardware tiers: Pirate Build (£660), Sovereign Edition (£25,000), Village Cluster (£150,000). Pricing page at `/pricing` (`templates/pricing.html`) with feature comparison table. Stripe integration via `routes/payments.py` and `routes/stripe_client.py` for checkout sessions and webhook handling. Feature gating enforced in upload routes (file size limits), encode routes (scatter mode restrictions), and journalism routes (`tier_required` decorator). Tier badge displayed in engine header. Upgrade toasts shown when blocked features are attempted.
...
- **stripe:** Stripe SDK for payment processing (checkout sessions, webhooks, customer management).
- **Standard Library:** Includes `zlib`, `lzma`, `wave`, `hashlib` for core functionalities.

## Database
- **PostgreSQL** (Replit built-in): Used for Void Messenger and Universal Auth. Tables: `users` (with `role`, `tier`, `tier_expires_at`, `stripe_customer_id`, `stripe_subscription_id` columns), `conversations`, `conversation_members`, `messages`. Messages are ChaCha20-Poly1305 encrypted before storage. Passwords hashed with Al-Jabr 286. Connection via `DATABASE_URL` env var.
