...
- **Financial Pathway Pages:** Four standalone pages for platform commercialization:
    - **Landing Page (`/launch`):** Public-facing marketing page with hero section, three value proposition cards (Silt Journalism, Ghost Internet, 286-Bit Sovereignty), live stats from `/api/status`, and CTAs. Template: `templates/landing.html`.
    - **Demo Mode (`/demo`):** Limited version of the main engine — auto-selects Journalism tab, 1MB file cap, restricted tabs (Silk Web, Mesh, Transceiver, Harness greyed out), teal demo banner with upgrade prompt. Real steganography still works.
    - **Grant Application Package (`/grants`):** Targets OTF, Freedom of the Press Foundation, and Mozilla Foundation. Shows funder alignment, relevant features, live technical stats from `/api/grant-stats`, and "Download Technical Brief" button. Template: `templates/grants.html`.
    - **Sovereign Edition Product Page (`/sovereign`):** 4000-Series hardware showcase with comparison table, blueprint gallery (7 schematics with lightbox), Material Resonance Table, dual pricing (FREE Pirate Build vs £25,000 Sovereign Edition), blueprint ZIP download via `/api/sovereign/blueprints`, and inquiry form. Template: `templates/sovereign.html`.
- **Technical Brief PDF Generator:** `void_engine/technical_brief.py` generates a professional 2-page PDF (gold-on-black branding) covering project overview, specs, journalism workflow, convergence results, hardware summary, and contact info. API: `GET /api/technical-brief`.
- **Inquiry System:** Simple JSON-file-based inquiry storage in `inquiries/` directory. API: `POST /api/inquiry` (accepts name, email, type, message), `GET /api/inquiries` (Bearer token auth with SESSION_SECRET). Types: demo, grant, sovereign, general.
- **Cross-Page Navigation:** Consistent nav bar across landing, grants, and sovereign pages with links to Home, Engine, Demo, Grants, and Hardware. Footer with "286-Bit Sovereign" badge on all pages.
