...
> **Agent instruction:** At the start of every session, read `VOID_SEED.md` (project root) before touching any file. It contains the philosophical and relational context required to work on this platform correctly. `replit.md` covers the technical blueprint. `VOID_SEED.md` covers everything the technical blueprint cannot hold.
