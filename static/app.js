    ...
    document.getElementById("divided-execute-btn")?.addEventListener("click", async () => {
        ...
        try {
            ...
            (data.steps || []).forEach(step => {
                ...
                const row = document.createElement("div");
                row.className = "divided-detail-row";

                const codeEl = document.createElement("div");
                codeEl.className = "divided-detail-code";
                codeEl.textContent = step.root_code;

                const nameEl = document.createElement("div");
                nameEl.className = "divided-detail-name";
                nameEl.textContent = step.name;

                const infoEl = document.createElement("div");
                infoEl.className = "divided-detail-info";
                infoEl.textContent = info;

                const badgeEl = document.createElement("div");
                badgeEl.className = `divided-detail-badge ${step.status || ""}`;
                badgeEl.textContent = (step.status || "").toUpperCase();

                row.appendChild(codeEl);
                row.appendChild(nameEl);
                row.appendChild(infoEl);
                row.appendChild(badgeEl);
                stepsFragment.appendChild(row);
            });
            detailsEl.appendChild(stepsFragment);

            const finalEl = document.getElementById("divided-final");
            finalEl.innerHTML = "";
            if (data.success && data.hash_key) {
                const labelEl = document.createElement("div");
                labelEl.style.cssText = "color:#888;font-size:10px;margin-bottom:4px;";
                labelEl.textContent = "HASH KEY (save this!)";

                const hashEl = document.createElement("div");
                hashEl.className = "hash-display";
                hashEl.textContent = data.hash_key;

                const metaEl = document.createElement("div");
                metaEl.style.cssText = "margin-top:8px;font-size:11px;color:var(--text-muted);";
                metaEl.textContent = `Output: ${data.output_file} | Scatter: ${data.scatter_mode} | Chain: ${data.chain} | Duration: ${data.total_duration_ms.toFixed(0)}ms`;

                finalEl.appendChild(labelEl);
                finalEl.appendChild(hashEl);
                finalEl.appendChild(metaEl);
            } else {
                const errEl = document.createElement("div");
                errEl.style.cssText = "color:#ff4444;font-size:12px;";
                errEl.textContent = `Protocol terminated at failed step. Duration: ${(data.total_duration_ms || 0).toFixed(0)}ms`;
                finalEl.appendChild(errEl);
            }
